...
class TigStyleRendererV2:
    ...
    def render_to_file(self, output_path: str):
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write(output)
        print(f"Git log rendered to: {output_path}")


def main():
    if len(sys.argv) < 2:
        print("Usage: python tig_graph_v2.py <repo_path> [output_file]")
        sys.exit(1)

    repo_path = sys.argv[1]
    output_file = sys.argv[2] if len(sys.argv) > 2 else 'git_log_v2.txt'
